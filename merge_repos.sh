...
# Copy and modified from http://choly.ca/post/git-merge-to-monorepo/
...
(
    cd $REPO_DIR_TMP &&
	git checkout ${EXTRA_BR} && git checkout master &&
...
git checkout -b ${EXTRA_BR} || git checkout ${EXTRA_BR}
git merge --allow-unrelated-histories "$REPO_NAME/${EXTRA_BR}"
